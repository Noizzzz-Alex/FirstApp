...
//  FirstApp
//
//  Created by Александр Харлампов on 13.12.2023.
//
...
final class DataService{
    ...
    func getData() -> [City] {
        var cities: [City] = []
        var index = 0
        var isExist = userDefaults.string(forKey: "name0")
        != nil
        while isExist {
            cities.append(City(name: userDefaults.string(forKey: "name" + String(index)) ?? "",
                               timezone: userDefaults.string(forKey: "timezone" + String(index)) ?? "",
                               coords: Coordinates(lat: userDefaults.double(forKey: "lat" + String(index)), lon: userDefaults.double(forKey: "lon" + String(index))),
